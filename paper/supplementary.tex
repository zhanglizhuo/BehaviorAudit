...
\caption{Threshold-sensitivity sweeps for the two operational cutoffs used in the audit. Panel~(a) varies the instability cutoff over $I \in [0.1, 3.0]$ and reports the number of datasets that would be flagged as failing split instability; the paper's choice of $I = 1.0$ flags one dataset (MM-TBA, with linear $I = 2.21$) and is stable to within $\pm 0.5$ of the cutoff. Panel~(b) varies the beat-rate cutoff over $[0.5, 1.0]$; the paper's choice of $0.90$ flags two datasets and the conclusion is robust for any cutoff in $[0.80, 0.95]$.}
