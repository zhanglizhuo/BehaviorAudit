...
\textbf{Response:} We thank Reviewer~3 for the detailed comments. Before addressing the substance, we provide a brief overview of our manuscript to correct what appears to be a mismatch in the review assignment.

\textbf{Our manuscript in brief:}
\begin{itemize}
  \item \textit{Title:} Limited Structural Reliability in Public Educational Prediction Benchmarks: A Four-Dimension Audit of Seven Datasets
  \item \textit{Keywords:} educational prediction, benchmark auditing, group-aware validation, dataset reliability, learning analytics
  \item \textit{Research object:} Seven public educational prediction datasets (N = 145 to 32,593), spanning K-12, higher education, entrance examination, and online learning.
  \item \textit{Method:} A four-dimension pre-modeling audit protocol: (1) baseline gap, (2) split instability, (3) null separation, (4) metadata adequacy via group-aware holdout. Models: linear regression, ridge, Random Forest, Gradient Boosting. No SHAP, LIME, ELI5, CatBoost, or class-imbalance treatment is involved.
  \item \textit{Conclusion:} Three of seven datasets pass all four checks. The dominant failure mode is cross-group fragility. Audit profiles are robust to metric choice.
\end{itemize}
