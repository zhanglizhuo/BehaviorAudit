...
Discussion, and Conclusion) and confirmed that the remaining uses of
``confirm'' appear in the group-holdout and train-test-gap analyses,
where the verb is supported by direct numerical evidence; those uses
are unchanged. The feature-attribution section now uses only
...
Throughout the rest of the manuscript (Methods, Results, Discussion,
Conclusion), references to ``Fragile datasets'' are retained because
they describe the full seven-dataset audit, which does include the
Fragile-profile MM-TBA. The scope restriction applies only to the
