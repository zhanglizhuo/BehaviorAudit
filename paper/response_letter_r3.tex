...
\begin{itemize}
\item The opening sentence now reads: ``To assess whether predictive
  signal reflects stable structure or split-specific noise, we track
  linear-model standardized coefficients and Random Forest feature
  importances across 100 repeated 80/20 splits.''
\item The concluding sentence---previously a negation---now reads:
  ``Because the feature-attribution analysis is conducted under iid random
  splits, it measures split-level attribution stability as a proxy for
  dataset readiness; evaluating attribution robustness under group-aware
  holdout is a natural next step.''
\end{itemize}

The section now states what the analysis does without apologising for
what it does not do.  (behavioraudit.tex, lines~318,~326)
...
\midrule
OULAD & 46 & $0.617 \pm 0.040$ & $0.777 \pm 0.081$ \\
...
(Supplementary Table~S8; behavioraudit.tex, lines~324--325)
...
\item \textbf{After:} ``Within each split, all features are Z-score
  standardised using parameters fitted exclusively on the training
  partition and applied to the corresponding test partition.''
\end{itemize}

The supplementary Table~S3 caption has been updated similarly.
(behavioraudit.tex, line~136; supplementary.tex, Table~S3 caption)
